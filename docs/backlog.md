...
- [x] **T-074** Aviso de privacidad: mostrar el texto en el registro (RF-37) — el contenido definitivo lo redacta el área jurídica de NovaUniversitas (RNF-11); mientras tanto usar un texto provisional claramente marcado como borrador. *Hecho con el texto provisional que entregó el equipo (2026-09-24), integrado tal cual en `app/lib/core/aviso_privacidad.dart`: se muestra íntegro en la propia pantalla de registro (no detrás de un enlace), en letra de cuerpo y antes de la casilla de aceptación, con el encabezado «BORRADOR — PENDIENTE DE VALIDACIÓN JURÍDICA» encima del texto (constante aparte, `avisoPrivacidadEncabezado`; el texto legal no se toca) y con el título de la tarjeta y la casilla marcados como borrador. La casilla es obligatoria (sin marcarla no se puede crear la cuenta, tampoco con «Reintentar») y lo que viaja como `acepto_aviso_privacidad` es su valor real. El contenido definitivo sigue pendiente de jurídica: al llegar, se reemplaza `avisoPrivacidadTexto` (y su copia en `registro_screen_test.dart`) y se quitan el encabezado de borrador y el «(borrador)» del título de la tarjeta y de la casilla.*
...
- **Texto legal del aviso de privacidad (T-074)**: el definitivo sigue pendiente de validación jurídica; T-074 usa el texto provisional (`app/lib/core/aviso_privacidad.dart`).
