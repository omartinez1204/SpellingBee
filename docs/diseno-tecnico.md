...
- **RNF-11 / RF-37** (aviso de privacidad, T-074): el texto vive en una sola constante, `app/lib/core/aviso_privacidad.dart`, y se muestra ÍNTEGRO en la propia pantalla de registro (no detrás de un enlace), en letra de cuerpo y antes de la casilla de aceptación; lo marcan como borrador el encabezado «BORRADOR — PENDIENTE DE VALIDACIÓN JURÍDICA» (constante aparte, `avisoPrivacidadEncabezado`, encima del texto), el título de la tarjeta y la casilla. Por ahora es un texto provisional entregado por el equipo e integrado tal cual — no se edita: el contenido definitivo lo redacta y valida el área jurídica de NovaUniversitas (RNF-11), y cuando llegue se reemplaza `avisoPrivacidadTexto` (y su copia en `registro_screen_test.dart`, que guarda el texto exacto a propósito) y se quitan el encabezado de borrador y el «(borrador)» del título y de la casilla. La casilla es obligatoria: sin marcarla el botón «Crear cuenta» está deshabilitado y «Reintentar» (RF-38) tampoco crea la cuenta; lo que viaja como `acepto_aviso_privacidad` es el valor de la casilla, y el backend (T-010) rechaza un `false` o su ausencia. Punto abierto, a confirmar con jurídica: el backend valida la aceptación pero no la guarda — no queda registro de cuándo se aceptó ni de qué versión del texto.
...
- **RNF-13** (respaldo SQLite, T-073): `backend/scripts/respaldar-bd.mjs` genera el respaldo con la Online Backup API de SQLite (`Database.prototype.backup()` de better-sqlite3 — el mismo mecanismo detrás de `sqlite3 <bd> ".backup <destino>"` en el CLI oficial), no con una copia de archivo directa: en modo WAL una escritura reciente puede vivir solo en `<bd>.db-wal`, todavía no en `<bd>.db`, y una copia cruda la perdería (lo reproduce `backend/scripts/respaldar-bd.spec.ts`). Comprueba la integridad del archivo (`PRAGMA integrity_check`) antes de darlo por bueno y no deja ningún archivo si falla. La política (semanal cada fin de semana; backend y base de datos en el mismo servidor; ejecutado y verificado por el equipo de desarrollo) está en `docs/operacion.md`, donde también queda marcado como pendiente el procedimiento de restauración, sin definir ni probar. Diferencia con el ERS, sin resolver aquí: RNF-13 recomienda como mínimo un respaldo diario y pide guardar la copia fuera del servidor de aplicación; la política indicada es semanal y no define dónde se guarda la copia.

## 5. Puntos que siguen abiertos (heredados del ERS, no de este documento)

Este diseño no resuelve, porque no le corresponde, los puntos que el ERS ya marca como pendientes en su §8.2 (servidor/dominio con TI, contenido legal del aviso de privacidad, catálogo incompleto de las 45 palabras). El backlog (`backlog.md`) los reconoce como bloqueadores de ciertas tareas, no los intenta resolver.

La periodicidad y quién hace el respaldo de SQLite (RNF-13) ya están documentadas en `docs/operacion.md` (T-073); allí mismo queda marcado como pendiente el procedimiento de restauración.
