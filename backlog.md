...
- [ ] **T-000** Inicializar monorepo con `/backend` (NestJS + TypeScript) y `/app` (Flutter). Configurar linter/formatter en ambos y un `README.md` raíz con cómo levantar cada uno.
- [ ] **T-001** Backend: configurar Prisma (o el ORM elegido) apuntando a SQLite en modo WAL (ERS §7). Crear el esquema inicial con las 6 entidades de `diseno-tecnico.md` §2 (Usuario, PerfilAlumno, Nivel, Palabra, RegistroPractica, Racha) y su primera migración.
...
- [ ] **T-030** Reproducción básica (RF-12), pausa/reanuda (RF-13), detener (RF-16), reproducciones ilimitadas (RF-17).
- [ ] **T-031** Retroceder/adelantar exactamente 5 segundos (RF-14, RF-15), sin pasar de 0 ni de la duración total.
...
- [ ] **T-040** Botón de inicio + cronómetro único combinado en 00:00 hasta que el alumno lo activa (RF-19), actualizado en tiempo real (RF-20).
- [ ] **T-041** Botón "Terminé" que detiene el cronómetro y fija el tiempo final (RF-21).
...
- [ ] **T-060** `GET /niveles/:id/descarga` (RF-31): paquete de palabras + audios de un nivel.
